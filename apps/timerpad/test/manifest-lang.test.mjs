...
test("manifest name / lang / start_url follow the requested language", () => {
  const cases = { en: "Timerpad", ko: "타이머패드", ja: "タイマーパッド", zh: "计时板" };
...
test("the Worker's first-HTML copy matches what React renders after mount", () => {
  for (const lang of ["ko", "en", "ja", "zh"]) {
    ...
    assert.equal(COPY[lang].image, `https://timerpad.try-dabble.com/og-image-${lang}.png`);
  }
});
...
test("the Korean shell in index.html matches the ko copy and carries the Naver meta", () => {
  ...
  assert.match(html, new RegExp(`class="tp-tagline">${rx(COPY.ko.tagline)}</p>`));
  ...
  assert.match(html, /<meta name="theme-color" content="#eef3f8" \/>/);
});
...
test("the fail-fix chips exist in every language", () => {
  ...
  assert.equal(I18N.en.chipNoLock, "Stop or reset mid-workout");
  assert.equal(I18N.en.chipHistoryFree, "Every timer is free");
...
test("every UI string exists in every language and zh never falls back to English", () => {
  ...
  const shared = new Set(["chipLangs", "fontMd", "fontLg", "fontXl"]);
