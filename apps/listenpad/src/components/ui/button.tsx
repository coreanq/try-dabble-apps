...
/**
 * Pills for the player: teal primary, amber for the A–B markers, sage for
 * success, card-white secondary. The shadcn API is untouched — only the
 * dress. Row buttons in wrapping flex rows use .lnp-btn instead.
 */
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center gap-1.5 rounded-full border font-bold whitespace-nowrap transition-transform duration-75 outline-none select-none touch-manipulation focus-visible:ring-3 focus-visible:ring-ring/40 active:translate-y-[1px] disabled:pointer-events-none disabled:opacity-40 aria-invalid:border-destructive [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "border-teal-deep/50 bg-teal-deep text-primary-foreground shadow-[0_2px_0_var(--color-teal-deep)] hover:brightness-105",
        amber:
          "border-amber-deep/50 bg-amber text-amber-ink shadow-[0_2px_0_var(--color-amber-deep)] hover:brightness-105",
