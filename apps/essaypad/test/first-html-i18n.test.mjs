 ...
 * ESSAYPAD_URL to point elsewhere; when nothing answers, every test here is
 * skipped so `npm test` stays a pure unit run.
 */
const BASE = process.env.ESSAYPAD_URL || "http://127.0.0.1:8788";
...
const skip = reachable ? false : `no server at ${BASE} (set ESSAYPAD_URL)`;
...
const CASES = [
  {
    ...
    ogLocale: "en_US",
    ogImage: "https://essaypad.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    ogLocale: "ja_JP",
    ogImage: "https://essaypad.try-dabble.com/og-image-ja.png",
  },
  {
    lang: "zh",
    htmlLang: "zh",
    ...
    ogLocale: "zh_CN",
    ogImage: "https://essaypad.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    ...
    ogLocale: "ko_KR",
    ogImage: "https://essaypad.try-dabble.com/og-image-ko.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} already has lang, title, banner, h1, tagline (no JS)`, { skip }, async () => {
    ...
    assert.match(first, new RegExp(`class="ep-tagline">${esc(c.tagline)}</p>`));
    ...
    assert.match(html, new RegExp(`rel="canonical" href="https://essaypad\\.try-dabble\\.com/\\?lang=${c.lang}"`));
  });
}
...
test("zh og:image is its own card, never the English file", { skip }, async () => {
  ...
  assert.match(html, /property="og:image" content="https:\/\/essaypad\.try-dabble\.com\/og-image-zh\.png"/);
});

test("the feedback widget is appended with data-app=essaypad, and nothing else is injected", { skip }, async () => {
  const res = await fetch(`${BASE}/`);
  const html = await res.text();
  assert.match(html, /<script src="https:\/\/try-dabble\.com\/widget\/feedback\.js" data-app="essaypad" defer><\/script>/);
...
test("the shell has no login, paywall, IAP, subscription trap or editor", { skip }, async () => {
  const res = await fetch(`${BASE}/?lang=en`);
  const html = await res.text();
  assert.doesNotMatch(html, /sign in|log in|premium plan|paywall|free trial|subscribe now|unlock pro|in-app purchase|upgrade to pro/i);
  assert.match(html, /No account\. No subscription\./);
  assert.match(html, /Unlimited essays are free/);
});
...
test("static PWA files are served", { skip }, async () => {
  ...
  for (const lang of ["ko", "en", "ja", "zh"]) {
    assert.match(sitemap, new RegExp(`https://essaypad\\.try-dabble\\.com/\\?lang=${lang}`));
  }
});
