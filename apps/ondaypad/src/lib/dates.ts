...
/* ---------- month-grid helpers (calendar, local) ---------- */
...
/** MM-DD, e.g. for grouping "this day across years". */
export function isMmDd(v: unknown): v is string {
  if (typeof v !== "string" || !/^\d{2}-\d{2}$/.test(v)) return false;
  const [m, d] = v.split("-").map(Number);
  return m >= 1 && m <= 12 && d >= 1 && d <= 31;
}

/** The MM-DD slice of a YYYY-MM-DD key. */
export function mmddOfDateKey(key: string): string {
  return key.slice(5);
}

/** Local midnight of the 1st of a YYYY-MM key. */
...
/** Number of days in a YYYY-MM key (28-31). */
export function daysInMonth(key: string): number {
  const [y, m] = key.split("-").map(Number);
  return new Date(y, m, 0).getDate();
}

/** 0 (Sun) .. 6 (Sat) weekday of the 1st of a YYYY-MM key. */
export function firstWeekdayOfMonth(key: string): number {
  return parseMonthKey(key).getDay();
}

/** Builds a YYYY-MM-DD key from a YYYY-MM key and a day of that month. */
export function dateKeyInMonth(month: string, day: number): string {
  return `${month}-${String(day).padStart(2, "0")}`;
}

export function formatMonthYear(key: string, locale: string): string {
  try {
    return new Intl.DateTimeFormat(locale, { year: "numeric", month: "long" }).format(parseMonthKey(key));
  } catch {
    return key;
  }
}

export function formatMmDd(mmdd: string, locale: string): string {
  const [m, d] = mmdd.split("-").map(Number);
  const probe = new Date(2027, m - 1, d, 0, 0, 0, 0);
  try {
    return new Intl.DateTimeFormat(locale, { month: "long", day: "numeric" }).format(probe);
  } catch {
    return mmdd;
  }
}
