...
const BASE = process.env.PETPLAY_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    htmlLang: "en",
    title: "PetPlay",
    tagline: "Free local digital pet — no generative AI. Build a sprite from parts or your photo (crop & tint on this device), name it, feed and play with visible needs, JSON backup. No account. No tokens. No ads.",
    localOnly: "Your data stays on this device. Nothing is sent to our servers.",
    ogLocale: "en_US",
    ogImage: "https://petplay.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    title: "ペットプレイ",
    tagline: "生成AIなしの無料ローカルデジタルペット。パーツと色、または自分の写真（端末で切り抜き＆色付け）でスプライトを作り、名前を付けてごはんと遊び、JSONバックアップ。アカウント不要。トークンなし。広告なし。",
    localOnly: "データはこの端末にだけ保存されます。サーバーには送りません。",
    ogLocale: "ja_JP",
    ogImage: "https://petplay.try-dabble.com/og-image-ja.png",
  },
  {
    lang: "zh",
    htmlLang: "zh",
    title: "宠玩",
    tagline: "无生成式 AI 的免费本地电子宠物。用部件和颜色，或用自己的照片（在本机裁剪上色）做精灵，起名并喂食玩耍，JSON 备份。无需账号。无代币。无广告。",
    localOnly: "数据仅保存在此设备，不会上传到服务器。",
    ogLocale: "zh_CN",
    ogImage: "https://petplay.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    title: "펫플레이",
    tagline: "생성형 AI 없는 무료 로컬 디지털 펫. 파츠·색으로 꾸미거나 내 사진으로 스프라이트(기기에서 자르고 색만 입힘), 이름 짓고 먹이·놀아주기, JSON 백업. 계정 없음. 토큰 없음. 광고 없음.",
    localOnly: "이 앱의 데이터는 이 기기에만 저장됩니다. 서버로 보내지 않습니다.",
    ogLocale: "ko_KR",
    ogImage: "https://petplay.try-dabble.com/og-image-ko.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} already has lang, title, banner, h1, tagline (no JS)`, async () => {
    ...
    assert.match(html, new RegExp(`rel="canonical" href="https://petplay\\.try-dabble\\.com/\\?lang=${c.lang}"`));
  });
}
...
test("zh og:image is its own card, never the English file", async () => {
  ...
  assert.match(html, /property="og:image" content="https:\/\/petplay\.try-dabble\.com\/og-image-zh\.png"/);
});

test("the feedback widget is appended with data-app=petplay, and nothing else is injected", async () => {
  const res = await fetch(`${BASE}/`);
  const html = await res.text();
  assert.match(html, /<script src="https:\/\/try-dabble\.com\/widget\/feedback\.js" data-app="petplay" defer><\/script>/);
...
test("manifest follows ?lang= and the td_lang cookie", async () => {
  ...
  assert.equal(en.name, "PetPlay");
  assert.equal(en.start_url, "/?lang=en");
  const ja = await (await fetch(`${BASE}/manifest.webmanifest`, { headers: { cookie: "td_lang=ja" } })).json();
  assert.equal(ja.name, "ペットプレイ");
  assert.equal(ja.lang, "ja");
});

test("the shell has no login, paywall, token, subscription pitch or AI-portrait promise", async () => {
  const res = await fetch(`${BASE}/?lang=en`);
  const html = await res.text();
  assert.doesNotMatch(html, /sign in|log in|premium|paywall|free trial|subscribe now|unlock pro|buy tokens|ai portrait/i);
  assert.match(html, /No account\. No tokens\. No ads\./);
  assert.match(html, /no generative AI/);
});
...
test("static PWA files are served", async () => {
  ...
  assert.match(sw, /"petplay-v1"/);
  const sitemap = await (await fetch(`${BASE}/sitemap.xml`)).text();
  for (const lang of ["ko", "en", "ja", "zh"]) {
    assert.match(sitemap, new RegExp(`https://petplay\\.try-dabble\\.com/\\?lang=${lang}`));
  }
});
