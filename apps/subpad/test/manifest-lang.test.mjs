...
test("manifest name / lang / start_url follow the requested language", () => {
  const cases = { en: "Subpad", ko: "섭패드", ja: "サブパッド", zh: "续费板" };
...
test("the Worker's first-HTML copy matches what React renders after mount", () => {
  for (const lang of ["ko", "en", "ja", "zh"]) {
    ...
    assert.equal(COPY[lang].image, `https://subpad.try-dabble.com/og-image-${lang}.png`);
  }
});
...
test("the Korean shell in index.html matches the ko copy and carries the Naver meta", () => {
  ...
  assert.match(html, new RegExp(`class="sb-tagline">${rx(COPY.ko.tagline)}</p>`));
  ...
  assert.match(html, /<meta name="theme-color" content="#f7f3ea" \/>/);
});

test("the fail-fix chips exist in every language", () => {
  const keys = ["chipNoAds", "chipUnlimited", "chipNoBank", "chipNoLoginWipe", "chipBackup", "chipCalendar", "chipFree", "chipLocal", "chipLangs"];
  for (const lang of ["ko", "en", "ja", "zh"]) {
  ...
  assert.equal(I18N.en.chipUnlimited, "Unlimited list");
  assert.equal(I18N.en.chipNoBank, "No bank link");
  assert.equal(I18N.en.chipNoLoginWipe, "No login wipe");
  assert.equal(I18N.en.chipBackup, "JSON backup");
  assert.equal(I18N.en.chipCalendar, "Calendar export");
