...
/**
 * Pills for the ledger sheet: sage primary, card-white secondary, a muted
 * coral for destructive. The shadcn API is untouched — only the dress. Row
 * buttons on the list are .sb-btn because they sit in wrapping flex rows.
 */
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center gap-1.5 rounded-full border font-bold whitespace-nowrap transition-transform duration-75 outline-none select-none touch-manipulation focus-visible:ring-3 focus-visible:ring-ring/40 active:translate-y-[1px] disabled:pointer-events-none disabled:opacity-40 aria-invalid:border-destructive [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "border-sage-deep/50 bg-sage text-primary-foreground shadow-[0_2px_0_var(--color-sage-deep)] hover:brightness-105",
        secondary:
        ...
        destructive:
          "border-sage/40 bg-sage-soft text-sage-ink shadow-[0_2px_0_rgba(232,93,76,0.25)] hover:brightness-95",
