...
/**
 * Clay pills for the kitchen: terracotta primary, cream secondary, a muted red
 * for destructive. The shadcn API is untouched — only the dress. The big
 * toolbar actions are not this component; they are .rl-btn, because nothing
 * else is that tall.
 */
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center gap-1.5 rounded-full border font-bold whitespace-nowrap transition-transform duration-75 outline-none select-none touch-manipulation focus-visible:ring-3 focus-visible:ring-ring/40 active:translate-y-[1px] disabled:pointer-events-none disabled:opacity-40 aria-invalid:border-destructive [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "border-terra-deep/40 bg-terra text-primary-foreground shadow-[0_2px_0_var(--color-terra-deep)] hover:brightness-105",
        secondary:
          "border-line-2 bg-cream text-ink shadow-[0_2px_0_rgba(51,38,29,0.08)] hover:bg-cream-2",
        outline:
        ...
        link: "border-transparent text-terra-deep underline-offset-4 shadow-none hover:underline active:translate-y-0",
      },
      size: {
