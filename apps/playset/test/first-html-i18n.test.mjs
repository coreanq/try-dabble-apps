...
const BASE = process.env.PLAYSET_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    chip: "No 3-game lock",
    ogLocale: "en_US",
    ogImage: "https://playset.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    chip: "3ゲーム制限なし",
    ogLocale: "ja_JP",
    ogImage: "https://playset.try-dabble.com/og-image-ja.png",
  },
  {
    ...
    chip: "没有3局限制",
    ogLocale: "zh_CN",
    ogImage: "https://playset.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    ...
    chip: "3게임 제한 없음",
    ogLocale: "ko_KR",
    ogImage: "https://playset.try-dabble.com/og-image.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} already has lang, title, banner, h1, chips, og image (no JS)`, async () => {
    ...
    assert.match(first, new RegExp(`class="ps-tagline">${esc(c.tagline)}</p>`));
    ...
    assert.match(
      html,
      new RegExp(`rel="canonical" href="https://playset\\.try-dabble\\.com/\\?lang=${c.lang}"`),
    );
  });
}
...
test("the feedback widget is appended for every language", async () => {
  ...
  assert.match(html, /data-app="playset"/);
});
