...
const BASE = process.env.POSEGUIDE_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    ogLocale: "en_US",
    ogImage: "https://poseguide.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    ogLocale: "ja_JP",
    ogImage: "https://poseguide.try-dabble.com/og-image-ja.png",
  },
  {
    lang: "zh",
    htmlLang: "zh",
    ...
    ogLocale: "zh_CN",
    ogImage: "https://poseguide.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    ...
    ogLocale: "ko_KR",
    ogImage: "https://poseguide.try-dabble.com/og-image-ko.png",
  },
];

/** The Worker writes text through HTMLRewriter, which escapes & as &amp;. */
function esc(s) {
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} already has lang, title, banner, h1, tagline (no JS)`, async () => {
    ...
    assert.match(first, new RegExp(`class="pg-tagline">${esc(c.tagline)}</p>`));
    ...
    assert.match(html, new RegExp(`rel="canonical" href="https://poseguide\\.try-dabble\\.com/\\?lang=${c.lang}"`));
  });
}
...
test("zh og:image is its own card, never the English file", async () => {
  ...
  assert.match(html, /property="og:image" content="https:\/\/poseguide\.try-dabble\.com\/og-image-zh\.png"/);
});

test("the feedback widget is appended with data-app=poseguide, and nothing else is injected", async () => {
  const res = await fetch(`${BASE}/`);
  const html = await res.text();
  assert.match(html, /<script src="https:\/\/try-dabble\.com\/widget\/feedback\.js" data-app="poseguide" defer><\/script>/);
  assert.equal(html.match(/widget\/feedback\.js/g).length, 1);
});

test("no ad script, login or paywall anywhere in the served HTML, in any language", async () => {
  for (const lang of ["", "ko", "en", "ja", "zh"]) {
    ...
    assert.doesNotMatch(html, /paywall|subscribe now|sign in to continue/i, `paywall in ?lang=${lang}`);
  }
});
...
test("static PWA files are served", async () => {
  ...
  assert.match(sw, /"poseguide-v1"/);
  const sitemap = await (await fetch(`${BASE}/sitemap.xml`)).text();
  for (const lang of ["ko", "en", "ja", "zh"]) {
    assert.match(sitemap, new RegExp(`https://poseguide\\.try-dabble\\.com/\\?lang=${lang}`));
  }
});
