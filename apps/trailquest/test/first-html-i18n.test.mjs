...
const BASE = process.env.TRAILQUEST_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    ogLocale: "en_US",
    ogImage: "https://trailquest.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    ogLocale: "ja_JP",
    ogImage: "https://trailquest.try-dabble.com/og-image-ja.png",
  },
  {
    lang: "zh",
    htmlLang: "zh",
    ...
    ogLocale: "zh_CN",
    ogImage: "https://trailquest.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    ...
    ogLocale: "ko_KR",
    ogImage: "https://trailquest.try-dabble.com/og-image-ko.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} already has lang, title, banner, h1, tagline (no JS)`, async () => {
    ...
    assert.match(first, new RegExp(`class="tq-tagline">${esc(c.tagline)}</p>`));
    ...
    assert.match(html, new RegExp(`rel="canonical" href="https://trailquest\\.try-dabble\\.com/\\?lang=${c.lang}"`));
  });
}
...
test("zh og:image is its own card, never the English file", async () => {
  ...
  assert.match(html, /property="og:image" content="https:\/\/trailquest\.try-dabble\.com\/og-image-zh\.png"/);
});

test("the feedback widget is appended with data-app=trailquest, and nothing else is injected", async () => {
  const res = await fetch(`${BASE}/`);
  const html = await res.text();
  assert.match(html, /<script src="https:\/\/try-dabble\.com\/widget\/feedback\.js" data-app="trailquest" defer><\/script>/);
...
test("the Naver Search Advisor meta is in the served head, in every language", async () => {
  for (const lang of ["", "ko", "en", "ja", "zh"]) {
    const res = await fetch(`${BASE}/${lang ? `?lang=${lang}` : ""}`);
    const html = await res.text();
    assert.match(html, /<meta name="naver-site-verification" content="cb50b4906a09539a5c0ede24022028167b4f7751"/, `naver in ?lang=${lang}`);
  }
});
...
test("static PWA files are served", async () => {
  ...
  assert.match(sw, /"trailquest-v1"/);
  const sitemap = await (await fetch(`${BASE}/sitemap.xml`)).text();
  for (const lang of ["ko", "en", "ja", "zh"]) {
    assert.match(sitemap, new RegExp(`https://trailquest\\.try-dabble\\.com/\\?lang=${lang}`));
  }
});
