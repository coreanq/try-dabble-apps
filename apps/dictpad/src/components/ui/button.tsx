...
/**
 * Pills for the notepad: sky primary, coral for the record control, sage for
 * success, card-white secondary. The shadcn API is untouched — only the
 * dress. Row buttons in wrapping flex rows use .dp-btn instead.
 */
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center gap-1.5 rounded-full border font-bold whitespace-nowrap transition-transform duration-75 outline-none select-none touch-manipulation focus-visible:ring-3 focus-visible:ring-ring/40 active:translate-y-[1px] disabled:pointer-events-none disabled:opacity-40 aria-invalid:border-destructive [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "border-sky-deep/50 bg-sky text-primary-foreground shadow-[0_2px_0_var(--color-sky-deep)] hover:brightness-105",
        coral:
          "border-coral-deep/50 bg-coral text-primary-foreground shadow-[0_2px_0_var(--color-coral-deep)] hover:brightness-105",
        sage: "border-sage-deep/50 bg-sage text-primary-foreground shadow-[0_2px_0_var(--color-sage-deep)] hover:brightness-105",
        secondary:
          "border-line-2 bg-card text-foreground shadow-[0_2px_0_rgba(15,23,42,0.08)] hover:bg-muted",
        outline:
        ...
        link: "border-transparent text-sky underline-offset-4 shadow-none hover:underline active:translate-y-0",
      },
      size: {
