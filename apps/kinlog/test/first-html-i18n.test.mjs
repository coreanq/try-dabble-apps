...
const BASE = process.env.KINLOG_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    chip: "No credit card",
    ogLocale: "en_US",
    ogImage: "https://kinlog.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    chip: "カード登録なし",
    ogLocale: "ja_JP",
    ogImage: "https://kinlog.try-dabble.com/og-image-ja.png",
  },
  {
    ...
    chip: "不要信用卡",
    ogLocale: "zh_CN",
    ogImage: "https://kinlog.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    ...
    chip: "카드 등록 없음",
    ogLocale: "ko_KR",
    ogImage: "https://kinlog.try-dabble.com/og-image.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} already has lang, title, banner, h1, chips, og image (no JS)`, async () => {
    ...
    assert.match(first, new RegExp(`class="kl-tagline">${esc(c.tagline)}</p>`));
    // The fail-fix has to be in the first HTML, not only after React mounts.
    assert.match(first, new RegExp(`id="chip-nocard"[^>]*>${esc(c.chip)}</li>`));
    ...
    assert.match(
      html,
      new RegExp(`rel="canonical" href="https://kinlog\\.try-dabble\\.com/\\?lang=${c.lang}"`),
    );
  });
}
...
test("the feedback widget is appended for every language", async () => {
  ...
  assert.match(html, /data-app="kinlog"/);
});
