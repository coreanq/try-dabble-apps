...
test("no leftover essay / hour / pay / sub / rec / dict product copy, and no word-count or deadline pace", () => {
  ...
  for (const f of files) {
    ...
    assert.doesNotMatch(
      text,
      /essaypad|에세이패드|hourpad|아워패드|paypad|페이패드|subpad|섭패드|recpad|dictpad|timerpad|affirmpad|listenpad|word count|words to go|targetWords|deadline|weekdaysOnly|check[- ]in|check[- ]out|overtime|timesheet|paycheck|\bep-|\bhp-|\bsb-|\btp-|\bdp-/i,
      path.relative(APP, f),
    );
  }
});
...
test("the feedback widget is a link only: nothing bundles it and there is no in-app feedback panel", () => {
  ...
  for (const f of files) {
    ...
    if (f.endsWith("og-lang.ts")) {
      ...
      assert.match(text, /const SLUG = 'progresspad'/);
      continue;
    }
