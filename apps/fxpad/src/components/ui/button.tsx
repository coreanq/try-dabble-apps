...
/**
 * Soft pills for the cabin: mint primary, cream secondary, a muted red for
 * destructive. The shadcn API is untouched — only the dress. The currency
 * chips and the refresh button are not this component; they are .fx-chip and
 * .fx-refresh, because nothing else is that tall.
 */
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center gap-1.5 rounded-full border font-bold whitespace-nowrap transition-transform duration-75 outline-none select-none touch-manipulation focus-visible:ring-3 focus-visible:ring-ring/40 active:translate-y-[1px] disabled:pointer-events-none disabled:opacity-40 aria-invalid:border-destructive [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "border-mint-deep/40 bg-mint text-primary-foreground shadow-[0_2px_0_var(--color-mint-deep)] hover:brightness-105",
        secondary:
          "border-line-2 bg-cream text-ink shadow-[0_2px_0_rgba(23,48,58,0.08)] hover:bg-cream-2",
        outline:
        ...
        link: "border-transparent text-mint-deep underline-offset-4 shadow-none hover:underline active:translate-y-0",
      },
      size: {
