...
const BASE = process.env.PHOTO_SPEC_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    ogLocale: "en_US",
    ogImage: "https://photo-spec.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    ogLocale: "ja_JP",
    ogImage: "https://photo-spec.try-dabble.com/og-image-ja.png",
  },
  {
    lang: "zh",
    htmlLang: "zh",
  ...
  },
  {
    ...
    ogLocale: "ko_KR",
    ogImage: "https://photo-spec.try-dabble.com/og-image.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} carries lang, title, banner, h1 and share tags (no JS)`, async () => {
    ...
    assert.match(
      first,
      new RegExp(
        `rel="canonical" href="${esc(`https://photo-spec.try-dabble.com/?lang=${c.lang}`)}"`,
      ),
    );
  });
}

test("the shared 의견 feedback widget is appended for photo-spec", async () => {
  const res = await fetch(`${BASE}/?lang=en`);
  const html = await res.text();
  assert.match(html, /https:\/\/try-dabble\.com\/widget\/feedback\.js/);
  assert.match(html, /data-app="photo-spec"/);
});
