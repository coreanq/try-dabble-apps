...
const BASE = process.env.LOCKPAD_URL || "http://127.0.0.1:8788";
...
const skip = reachable ? false : `no server at ${BASE} (set LOCKPAD_URL)`;
...
for (const lang of LANGS) {
  test(`?lang=${lang} rewrites title, banner, tagline and og`, { skip }, async () => {
    ...
    assert.match(html, new RegExp(`class="lp-tagline">${esc(c.tagline)}</p>`));
    ...
    assert.match(html, new RegExp(`rel="canonical" href="https://lockpad\\.try-dabble\\.com/\\?lang=${lang}"`));
    assert.doesNotMatch(html, /ca-pub-|adsbygoogle/i);
  });
}

test("the feedback widget is appended with data-app=lockpad, and nothing else is injected", { skip }, async () => {
  const html = await (await fetch(`${BASE}/?lang=en`)).text();
  assert.match(html, /<script src="https:\/\/try-dabble\.com\/widget\/feedback\.js" data-app="lockpad" defer><\/script>/);
...
test("static assets name lockpad", () => {
  const sw = readFileSync(new URL("../public/sw.js", import.meta.url), "utf8");
  const sitemap = readFileSync(new URL("../public/sitemap.xml", import.meta.url), "utf8");
  assert.match(sw, /"lockpad-v1"/);
  for (const lang of LANGS) {
    assert.match(sitemap, new RegExp(`https://lockpad\\.try-dabble\\.com/\\?lang=${lang}`));
  }
});
