...
맞춘다. 결과 요약과 편차 근거는 커밋된 `fit-report.txt`에 있다(총 14160석, 58구역, 12구역 플래그).
...
배치도의 86 %만 있는 31·32·35·36구역은 이웃 구역을 `like`로 본떴다. 플래그 12개는 모두
좌석 수가 아니라 `maxErr > 1.0 m`이며, 원인은 구역 안 포탈·가로통로 때문에 열마다 각도 폭이 다른 것이다
(피팅 5112석 중 1 m 초과 146석 = 2.9 %). 자세한 내용은 `fit-report.txt` 아래쪽 메모.
