...
const BASE = process.env.LATER_INBOX_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    ogLocale: "en_US",
    ogImage: "https://later-inbox.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    ogLocale: "ja_JP",
    ogImage: "https://later-inbox.try-dabble.com/og-image-ja.png",
  },
  {
    lang: "zh",
    htmlLang: "zh",
    ...
    ogLocale: "zh_CN",
    ogImage: "https://later-inbox.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    ...
    ogLocale: "ko_KR",
    ogImage: "https://later-inbox.try-dabble.com/og-image.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} carries lang, title, banner, h1 and share tags (no JS)`, async () => {
    ...
    assert.match(
      first,
      new RegExp(
        `rel="canonical" href="${esc(`https://later-inbox.try-dabble.com/?lang=${c.lang}`)}"`,
      ),
    );
  });
}

test("the shared 의견 feedback widget is appended for later-inbox", async () => {
  const res = await fetch(`${BASE}/?lang=en`);
  const html = await res.text();
  assert.match(html, /https:\/\/try-dabble\.com\/widget\/feedback\.js/);
  assert.match(html, /data-app="later-inbox"/);
});
