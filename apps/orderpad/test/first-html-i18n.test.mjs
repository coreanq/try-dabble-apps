...
const BASE = process.env.ORDERPAD_URL || "http://127.0.0.1:8788";
...
const CASES = [
  {
    ...
    chip: "No Meta connect",
    ogLocale: "en_US",
    ogImage: "https://orderpad.try-dabble.com/og-image-en.png",
  },
  {
    lang: "ja",
    htmlLang: "ja",
    ...
    chip: "Meta連携なし",
    ogLocale: "ja_JP",
    ogImage: "https://orderpad.try-dabble.com/og-image-ja.png",
  },
  {
    ...
    chip: "不接 Meta 账号",
    ogLocale: "zh_CN",
    ogImage: "https://orderpad.try-dabble.com/og-image-zh.png",
  },
  {
    lang: "ko",
    htmlLang: "ko",
    ...
    chip: "메타 연동 없음",
    ogLocale: "ko_KR",
    ogImage: "https://orderpad.try-dabble.com/og-image.png",
  },
];
...
for (const c of CASES) {
  test(`first HTML ?lang=${c.lang} already has lang, title, banner, h1, chips, og image (no JS)`, async () => {
    ...
    assert.match(first, new RegExp(`class="op-tagline">${esc(c.tagline)}</p>`));
    // The fail-fix has to be in the first HTML, not only after React mounts.
    assert.match(first, new RegExp(`id="chip-nometa"[^>]*>${esc(c.chip)}</li>`));
    ...
    assert.match(
      html,
      new RegExp(`rel="canonical" href="https://orderpad\\.try-dabble\\.com/\\?lang=${c.lang}"`),
    );
  });
}
...
test("the feedback widget is appended for every language", async () => {
  ...
  assert.match(html, /data-app="orderpad"/);
});
