...
- [dictpad](https://dictpad.try-dabble.com/) — local talk-to-text notepad: Web Speech live dictation into editable named notes, voice punctuation (period / comma / new line / paragraph in ko/en/ja/zh), recognition-language picker, copy / .txt / JSON backup; Vite + React + TS, `npm run deploy`
- [listenpad](https://listenpad.try-dabble.com/) — local language-listening drill player: pick one audio file, one-tap 1 / 2 / 3 / 4 s rewind and skip, A–B loop, 0.5×–1.5× speed, scrubber, listening notes per file name, JSON backup (no audio), ko/en/ja/zh; Vite + React + TS, `npm run deploy`
